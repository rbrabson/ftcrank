...
build-windows:
	...
	$(GO) build -v --ldflags="-w -X main.Version=$(VERSION) -X main.Revision=$(REVISION)" \
		-o bin/windows/amd64/update cmd/update/main.go  # windows
...
build-linux:
	$(GO) build -v --ldflags="-w -X main.Version=$(VERSION) -X main.Revision=$(REVISION)" \
		-o bin/linux/amd64/rank cmd/rank/main.go  # linux
	$(GO) build -v --ldflags="-w -X main.Version=$(VERSION) -X main.Revision=$(REVISION)" \
		-o bin/linux/amd64/predict cmd/predict/main.go  # linux
	$(GO) build -v --ldflags="-w -X main.Version=$(VERSION) -X main.Revision=$(REVISION)" \
		-o bin/linux/amd64/update cmd/update/main.go  # linux
...
build-mac-amd:
	$(GO) build -v --ldflags="-w -X main.Version=$(VERSION) -X main.Revision=$(REVISION)" \
		-o bin/macos/amd64/rank cmd/rank/main.go  # mac osx intel chip
	$(GO) build -v --ldflags="-w -X main.Version=$(VERSION) -X main.Revision=$(REVISION)" \
		-o bin/macos/amd64/predict cmd/predict/main.go  # mac osx intel chip
	$(GO) build -v --ldflags="-w -X main.Version=$(VERSION) -X main.Revision=$(REVISION)" \
		-o bin/macos/amd64/update cmd/update/main.go  # mac osx intel chip
...
build-mac-arm:
	$(GO) build -v --ldflags="-w -X main.Version=$(VERSION) -X main.Revision=$(REVISION)" \
		-o bin/macos/arm64/rank cmd/rank/main.go  # mac osx arm chip
	$(GO) build -v --ldflags="-w -X main.Version=$(VERSION) -X main.Revision=$(REVISION)" \
		-o bin/macos/arm64/predict cmd/predict/main.go  # mac osx arm chip
	$(GO) build -v --ldflags="-w -X main.Version=$(VERSION) -X main.Revision=$(REVISION)" \
		-o bin/macos/arm64/update cmd/update/main.go  # mac osx arm chip
